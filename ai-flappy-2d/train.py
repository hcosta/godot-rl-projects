"""
Docstring for train
note: --viz (visualization)
classic mode: python train.py --save_checkpoint_frequency=5000 --onnx_export_path=model.onnx --viz   
turbo headless en exe: python train.py --env_path="build/game.exe" --n_parallel=4 --speedup=200 --save_checkpoint_frequency=25000 --onnx_export_path=model.onnx
turbo inference test en exe: python train.py --inference --env_path="build/game.exe" --resume_model_path "logs/sb3/experiment_cp/experiment_300000_steps.zip" --viz --speedup 1
"""
...
def cleanup():
    ...
    try:
        ...
        print("Error exporting to ONNX, but the .zip is safe:", e)
    close_env()


# Sync our checkpoint naming with Stable Baselines 3's automatic tensorboard log naming
match = re.match(r"^(.*)_(\d+)$", args.experiment_name)
if match:
    # Remove any manually added number suffix so SB3 doesn't create e.g. "experiment_1_1"
    args.experiment_name = match.group(1)

base_name = args.experiment_name
run_num = 1
while True:
    tb_dir = os.path.join(args.experiment_dir, f"{base_name}_{run_num}")
    cp_dir = os.path.join(args.experiment_dir, f"{base_name}_{run_num}_cp")
    if not os.path.isdir(tb_dir) and not os.path.isdir(cp_dir):
        break
    run_num += 1

checkpoint_name_prefix = f"{base_name}_{run_num}"
path_checkpoint = os.path.join(args.experiment_dir, f"{checkpoint_name_prefix}_cp")
...
if args.resume_model_path is None:
    ...
    model: PPO = PPO(
        ...
        ent_coef=0.0001,
        verbose=2,
        n_steps=1024 // args.n_parallel,  # OJO CON ESTE PODRIA FALLAR??? MIRARLO MAÑANA
        batch_size=64,        
